# SEC XBRL Automated Financial Analysis Reports

A lightweight tool that pulls public financial statement data from the SEC (XBRL Company Facts), normalizes it, computes key financial metrics, generates charts, and outputs automated reports plus a Tableau-ready SQLite database.

---

## Features

- Fetch SEC public company data by ticker
  - Ticker → CIK
  - CIK → submissions
  - CIK → companyfacts (XBRL facts)
- Normalize raw XBRL facts into a clean, analysis-friendly table
- Build an annual financials table for the last N fiscal years
- Compute common metrics
  - Revenue, Gross Profit, Operating Income, Net Income
  - CFO, Capex, Free Cash Flow
  - Revenue YoY, margins, ROE
- Generate a Markdown report per ticker
  - Highlights
  - Charts grid
  - Annual financials table
  - Optional XBRL concept map
- Export results into a SQLite database for BI tools like Tableau

---

## Project Structure

Common files in this repo:

- `sec_client.py`  
  SEC HTTP client (ticker to CIK, submissions, companyfacts) with caching and retries.

- `xbrl_normalize.py`  
  Normalize SEC companyfacts JSON into a pandas DataFrame.

- `financials.py`  
  Build annual financials table and derived metrics (YoY, margins, ROE, etc).

- `viz.py`  
  Generate and save charts for the report.

- `report.py`  
  Generate Markdown reports (single ticker or multiple tickers) and an index page.

- `export_sqlite.py`  
  Export annual metrics into a SQLite database for Tableau dashboards.


---

## Usage

### 1) Markdown Reports

Generate a report:

```bash
python report.py --ticker AAPL --years 5
```

Or adjust years:

```bash
python report.py --ticker AAPL --years 10
```

Or multiple:

```bash
python report.py --tickers AAPL,MSFT,NVDA --years 5
```

Typical outputs:

* `output_reports/AAPL.md`

### 2) Export to SQLite (Tableau-ready)

Export one ticker:

```bash
python export_sqlite.py --ticker AAPL --years 5
```

Or adjust years:

```bash
python export_sqlite.py --ticker AAPL --years 10
```

Or multiple:

```bash
python export_sqlite.py --tickers AAPL,MSFT,NVDA --years 5
```

Outputs:
* `outputs/sec_fin.db`

---

## Tableau Connection

1. Open Tableau Desktop
2. **Connect → To a File → SQLite**
3. Select: `outputs/sec_fin.db`
4. Choose table: `annual_long`
5. Build dashboards using:

   * `fy` on Columns (time)
   * `value` on Rows
   * `metric` on Color / Filter
   * `ticker` on Filter (for multi-company comparisons)

---

## Notes and Limitations

* Data is sourced from SEC XBRL Company Facts and depends on issuer filings.
* Fiscal year definitions vary by company. The report uses the company’s own fiscal year end.
* Some concepts may change across years; the project uses a concept map to select the best available metric.

---

## Example Report

Below is an example output for Apple (AAPL).

### Apple Inc. - AAPL Automated Financial Analysis Report

- CIK: `0000320193`
- Generated: 2026-01-12 00:53:47
- Coverage: last 5 fiscal years

#### Highlights
- Latest fiscal year: **FY2025** ended 2025-09-27.
- Revenue: **416.2B** and 6.4% YoY.
- Gross margin: **46.9%**.
- Operating margin: **32.0%**.
- Net margin: **26.9%**.
- Free cash flow: **98.8B** and 23.7% of revenue.
- Revenue increased vs prior year.

#### Charts

| | |
|---|---|
| <b>Revenue</b><br><img src='output_reports/assets/AAPL/revenue.png' width='100%'> | <b>Revenue YoY</b><br><img src='output_reports/assets/AAPL/revenue_yoy.png' width='100%'> |
| <b>Margins</b><br><img src='output_reports/assets/AAPL/margins.png' width='100%'> | <b>Cash flow</b><br><img src='output_reports/assets/AAPL/cash_flow.png' width='100%'> |
| <b>Income statement levels</b><br><img src='output_reports/assets/AAPL/income_statement.png' width='100%'> | <b>Balance sheet snapshot</b><br><img src='output_reports/assets/AAPL/balance_sheet.png' width='100%'> |
| <b>Cash flow quality</b><br><img src='output_reports/assets/AAPL/cash_quality.png' width='100%'> | <b>Return on equity</b><br><img src='output_reports/assets/AAPL/roe.png' width='100%'> |

#### Annual Financials Table
USD in billions for level metrics
|   fy | fiscal_year_end   |   revenue |   gross_profit |   operating_income |   net_income |   cfo |   capex |   fcf | revenue_yoy   | gross_margin   | operating_margin   | net_margin   | fcf_margin   |   cash |   equity |
|-----:|:------------------|----------:|---------------:|-------------------:|-------------:|------:|--------:|------:|:--------------|:---------------|:-------------------|:-------------|:-------------|-------:|---------:|
| 2025 | 2025-09-27        |     416.2 |          195.2 |              133.1 |        112   | 111.5 |    12.7 |  98.8 | 6.4%          | 46.9%          | 32.0%              | 26.9%        | 23.7%        |   35.9 |     73.7 |
| 2024 | 2024-09-28        |     391   |          180.7 |              123.2 |         93.7 | 118.3 |     9.4 | 108.8 | 2.0%          | 46.2%          | 31.5%              | 24.0%        | 27.8%        |   29.9 |     57   |
| 2023 | 2023-09-30        |     383.3 |          169.1 |              114.3 |         97   | 110.5 |    11   |  99.6 | -2.8%         | 44.1%          | 29.8%              | 25.3%        | 26.0%        |   30   |     62.1 |
| 2022 | 2022-09-24        |     394.3 |          170.8 |              119.4 |         99.8 | 122.2 |    10.7 | 111.4 | 7.8%          | 43.3%          | 30.3%              | 25.3%        | 28.3%        |   23.6 |     50.7 |
| 2021 | 2021-09-25        |     365.8 |          152.8 |              108.9 |         94.7 | 104   |    11.1 |  93   | NA            | 41.8%          | 29.8%              | 25.9%        | 25.4%        |   34.9 |     63.1 |

#### XBRL Concept Map

| Metric | XBRL Concept |
|---|---|
| revenue | `RevenueFromContractWithCustomerExcludingAssessedTax` |
| gross_profit | `GrossProfit` |
| operating_income | `OperatingIncomeLoss` |
| net_income | `NetIncomeLoss` |
| cfo | `NetCashProvidedByUsedInOperatingActivities` |
| capex | `PaymentsToAcquirePropertyPlantAndEquipment` |
| cash | `CashAndCashEquivalentsAtCarryingValue` |
| equity | `StockholdersEquity` |